...
@WebMvcTest(
        value = WalkCourseController.class,
...
@Import(WalkCourseControllerTest.TestSecurityConfig.class)
@DisplayName("WalkCourseController Web MVC 테스트")
class WalkCourseControllerTest {
    ...
    @MockBean WalkCourseService walkCourseService;
    ...
    @Test
    @DisplayName("산책 코스 추천 - dogId 없이 200 OK")
    void recommend_dogId없음_200() throws Exception {
        CourseRecommendResponse resp = CourseRecommendResponse.builder()
                ...
                .build();
        when(walkCourseService.recommend(37.796, 128.900, 1L, null)).thenReturn(List.of(resp));

        mockMvc.perform(get("/api/v1/walk-courses")
                        .param("lat", "37.796").param("lng", "128.900")
                        .with(authAs(1L)))
    ...
    @Test
    @DisplayName("산책 코스 추천 - dogId 포함 200 OK")
    void recommend_dogId포함_200() throws Exception {
        CourseRecommendResponse resp = CourseRecommendResponse.builder()
                ...
                .build();
        when(walkCourseService.recommend(37.796, 128.900, 1L, 5L)).thenReturn(List.of(resp));

        mockMvc.perform(get("/api/v1/walk-courses")
                        .param("lat", "37.796").param("lng", "128.900").param("dogId", "5")
                        .with(authAs(1L)))
                .andExpect(status().isOk())
    ...
    void recommend_빈_결과() throws Exception {
        when(walkCourseService.recommend(37.796, 128.900, 1L, null)).thenReturn(List.of());
    ...
    void recommend_없는_반려견_404() throws Exception {
        when(walkCourseService.recommend(37.796, 128.900, 1L, 99L))
                .thenThrow(new NoSuchElementException("반려견을 찾을 수 없습니다. id=99"));

        mockMvc.perform(get("/api/v1/walk-courses")
                        .param("lat", "37.796").param("lng", "128.900").param("dogId", "99")
                        .with(authAs(1L)))
    ...
    void recommend_파라미터_누락_400() throws Exception {
        mockMvc.perform(get("/api/v1/walk-courses").with(authAs(1L)))
